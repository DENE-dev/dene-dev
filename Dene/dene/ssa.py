...
from ..core.vars_visitor  import get_vars
from ..util import ExprVisitor,get_condition_constraints
...
BUILT_IN_FUNCTIONS = { 
        ...
        # Others
        "NotImplemented", "__main__", "__doc__",  "__name__", "__debug__", "__class__", "__name__"
...
class Dene:
    ...
    def compute_undefined_names(self, cfg, scope=["mod"]):
        ...
        final_block_id = all_blocks[-1].id
        ...
        for block in all_blocks:
            ...
            # process this block
            block_id = block.id
            all_used_idents = block_ident_use[block_id]
            if block_id == final_block_id: 
                #print(subscope_undefined_names, "------")
                for su_name in subscope_undefined_names:
                    
                    if su_name["name"] not in block_ident_gen[block_id]:

                        all_used_idents.append(su_name)
            ...
            # if this block only has zero predecessors, then go to next block
            if len(block.predecessors) == 0:
                undefined_names += idents_left 
                continue 
            ...
            for ident in idents_left:
                ...
                for pred in block.predecessors:
                    #i_consts = get_condition_constraints(pred.exitcase)
                    i_consts = []
                    is_not_found = self.backward_query(block, pred, ident["name"], visited, dom=dom, path=exec_path, block_ident_gen=block_ident_gen, block_ident_del=block_ident_del, immediate_constraints = i_consts)
                    if not is_not_found:
                        # it has been found
    ...
    def backward_query_new(self, block, ident_name, visited, path = [], dom={},idom = {}, dom_stmt_res = [],  block_ident_gen={},  block_ident_del= {}, entry_id=1):
        ...
        for suc_link in block.predecessors:
            if isinstance(suc_link.exitcase, ast.NameConstant) and suc_link.exitcase.value == False:
                continue

            # to refactor this part. 
            # put the parent block as the first one 
            # which link/exit shoud be feed to the query interface so that we can determine 
            ...
            parent_block_func_calls = parent_block.func_calls
            parent_block_func_calls = [tmp["name"] for tmp in parent_block_func_calls]
            
            if "sys.exit" in parent_block_func_calls:
                continue
            if "exit" in parent_block_func_calls:
                continue
    ...
    def backward_query(self, block, link, ident_name, visited, path = [], dom={}, idom = {}, dom_stmt_res = [],  block_ident_gen={},  block_ident_del= {}, immediate_constraints=[]):
        # condition constraints:
        # the backward algorithms has the following steps:
        # if this block does not contian the definition of the name, then we will continue to backward to its all predecessors
        # if this block has the definition, return TRUE
        # if this block has exit then stop visiting because this path does not exist
        # if this block has been visited then stop visiting it anymore
        # if this block has no parent return False. 
        visited.add(block.id)
        ...
        # TRUE means, we can find it 
        direct_constraints = link.exitcase
        str_constraints = get_condition_constraints(direct_constraints)
        #print(str_constraints)

        # immediate constraints should be statisfied.
        #if all constraintws of immediate constraints are included in the direct constarints then we can 
       
        if any(cons not in str_constraints for cons in immediate_constraints):
            return True

        if isinstance(direct_constraints, ast.NameConstant) and direct_constraints.value == False:
            return True
 
        # put the parent block as the first one 
        # which link/exit shoud be feed to the query interface so that we can determine 
        # the source of the link is parent block, will lookup this parent block
        parent_block = link.source
        
        # deal with cycles, this is back edge   
        parent_block_func_calls = parent_block.func_calls
        parent_block_func_calls = [tmp["name"] for tmp in parent_block_func_calls]
        
        # encounter some system exit functions 
        if "sys.exit" in parent_block_func_calls:
            return True
        ...
        # not in its parent gen set  then search from this path
        ret_every_plink = []
        # name not found yet but reached the entry block
        if len(parent_block.predecessors) == 0:
            return False

        for plink in parent_block.predecessors:
            i_consts = get_condition_constraints(plink.exitcase)
        
            ret = self.backward_query(parent_block, plink, ident_name, visited, dom=dom, block_ident_gen=block_ident_gen,  path = path, immediate_constraints = immediate_constraints)
            # if it can be found then return True, otherwise return false
            if ret == False:
                return False
